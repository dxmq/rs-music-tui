...
pub fn handler(key: Key, app: &mut App) {
    match key {
        ...
        k if common_key_events::down_event(k) => {
            let next_index = common_key_events::on_down_press_handler(&app.artists, Some(app.artists_selected_index));
            app.artists_selected_index = next_index;
        }
        k if common_key_events::up_event(k) => {
            let next_index = common_key_events::on_up_press_handler(&app.artists, Some(app.artists_selected_index));
            app.artists_selected_index = next_index;
        }
